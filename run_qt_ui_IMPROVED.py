...
def main(argv: list[str]) -> int:
    ...
    # *** IMPROVED VERSION: Same visual layout but with proper layouts ***
    ui_path = Path(__file__).with_name("ui") / "password_vault_IMPROVED.ui"
    print(f"Loading IMPROVED UI from: {ui_path}")
    window = load_ui(ui_path)

    # Load HIGH-RES title image and let Qt scale it for display
    title_label = window.findChild(QtWidgets.QLabel, "lblTitle")
    if title_label is not None:
        # Load the high-resolution image (7144×1224)
        high_res_pixmap = QtGui.QPixmap(str(Path(__file__).parent / "TEMPSVG" / "PHIGEN_TITLE_FINAL.png"))

        if not high_res_pixmap.isNull():
            # Scale for display using Qt's high-quality smooth transformation
            # The label height is 61px, so calculate width maintaining aspect ratio
            display_height = 61
            ...
            display_pixmap = high_res_pixmap.scaled(
                ...
                QtCore.Qt.TransformationMode.SmoothTransformation  # High-quality downsampling
            )

            title_label.setPixmap(display_pixmap)
            title_label.setText("")  # Clear any existing text
            title_label.setAlignment(QtCore.Qt.AlignmentFlag.AlignCenter)
            print(f"[OK] Loaded high-res title: {high_res_pixmap.width()}x{high_res_pixmap.height()} -> scaled to {display_pixmap.width()}x{display_pixmap.height()} for display")
        else:
            print("[WARNING] Could not load PHIGEN_TITLE_FINAL.png")
    ...
    print("\n" + "="*70)
    print("IMPROVED UI - Same Look, Better Responsiveness!")
    print("="*70)
    print("This version keeps your original layout exactly:")
    print("  - Title at top (centered)")
    print("  - Length controls on left")
    print("  - Association and Password inputs")
    print("  - Generate/Save/Folder buttons in a row")
    print("  - 2x4 button grid on LEFT")
    print("  - List widget on RIGHT")
    print("")
    print("BUT with layout improvements:")
    print("  + Input fields expand with window width")
    print("  + Button grid stays perfectly aligned")
    print("  + List widget grows/shrinks smoothly")
    print("  + Everything scales proportionally")
    print("="*70 + "\n")
