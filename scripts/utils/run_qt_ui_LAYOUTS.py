...
def main(argv: list[str]) -> int:
    ...
    # *** MODIFIED: Load the LAYOUTS version of the UI ***
    ui_path = Path(__file__).with_name("ui") / "password_vault_LAYOUTS.ui"
    print(f"Loading UI with layouts from: {ui_path}")
    ...
    # Add window title to make it clear which version is running
    window.setWindowTitle("Password Vault - PhiGEN (Qt UI with Layouts) - TEST VERSION")

    print("\n" + "="*70)
    print("TESTING: UI with Layouts")
    print("="*70)
    print("Try resizing the window to see the difference!")
    print("- Input fields should expand with window width")
    print("- Button grid should maintain perfect alignment")
    print("- List widget should grow/shrink smoothly")
    print("="*70 + "\n")
